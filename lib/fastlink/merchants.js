 ...
 *
 * The pickup detail endpoint allows PATCH, so:
 *   - force=false (approval/backfill): skip if a pickup id already exists
 *   - force=true  (vendor edited pickup): PATCH the existing record in place
 *
 * Creating a fresh pickup instead would leave the old one behind with
 * is_default still true, giving the merchant two default pickup points.
 *
...
export async function syncPickupAddress(admin, vendorId, { force = false } = {}) {
  ...
  if ((lat == null || lng == null) && config.geocoding.enabled && addressText) {
    ...
    //   how a vendor in Ibadan got a pickup point in Lagos. A wrong point can be
    //   corrected now that PATCH is available, but it silently misroutes riders
    //   until someone notices. No pickup is the better failure; the vendor sets
    //   one from the map.
  ...
  // Update the existing record rather than adding a second default.
  const pickup = vendor.fastlink_pickup_id
    ? await fastlink.updatePickupAddress(vendor.fastlink_pickup_id, payload)
    : await fastlink.createPickupAddress(payload);

  const pickupId = vendor.fastlink_pickup_id
    ? String(vendor.fastlink_pickup_id)
    : pickup?.id != null ? String(pickup.id) : null;
  await admin
    .from("vendors")
    .update({
