...
export async function quoteShippingForItems({ admin, destination, items, allowGeocode = true }) {
  ...
  for (const vid of vendorIds) {
    ...
    try {
      ...
      // Fast Link answers with a well-formed quote of 0.00 — right currency,
      // right distance — when the platform's pricing formula is unconfigured.
      // Taken at face value that is free delivery for every customer, and it
      // overwrites the zone fee server-side. Zone pricing is the safe reading.
      const raw = Number(res?.cost);
      if (!Number.isFinite(raw) || raw <= 0) {
        console.error(
          `[fastlink] non-chargeable shipping quote for vendor ${vid}: cost=${JSON.stringify(res?.cost)}`,
        );
        return { fallback: true, reason: "provider_zero_price", vendorId: vid };
      }
