...
export async function dispatchOrder(admin, orderId) {
  ...
  try {
    ...
    // Reading flOrder.id directly returned null and sent us on a needless lookup.
    const created = flOrder?.order ?? flOrder;
    let flId = created?.id ?? created?.order_number ?? null;

    // Kept as a fallback in case the reply ever arrives without a body.
    ...
    await admin
      ...
      .update({
        ...
        fastlink_status:         created?.status ?? "pending",
