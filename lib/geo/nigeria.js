...
/**
 * Reduce a state name to a single key regardless of how it was written.
 *
 * The same state reaches us three ways — a vendor's stored `state`, a dropdown
 * label, and Mapbox's `region.name` on a reverse geocode — and they disagree on
 * casing, on "State" suffixes, and on what to call the capital territory.
 *
 * @param {string} name
...
export function canonicalState(name) {
  ...
  if (!key) return null;
  key = key.replace(/\s+state$/, "").trim();
  const resolved = ALIASES[key] ?? key;
  return resolved in STATE_CENTRES ? resolved : null;
}

/**
 * Do two state names refer to the same state?
 *
 * Returns true when either side is unrecognised: that is missing information,
 * not evidence of a mismatch, and warning on it would cry wolf at every vendor
 * whose state was never captured.
 */
export function sameState(a, b) {
  const ca = canonicalState(a);
  const cb = canonicalState(b);
